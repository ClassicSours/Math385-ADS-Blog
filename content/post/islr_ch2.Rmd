...
<h4>
  (d) The variance of the error terms, i.e. σ<sup>2</sup> = `Var( )`, is extremely high.
</h4>

> Since the σ<sup>2</sup> is so high the inflexible method will outperform the flexible method because it will mimic the large variance and increase the variance in the results that it predicts as well.
...
<h4> (b) Make some pairwise scatterplots of the predictors (columns) in the dataset.  Describe your findings </h4>
...
gg.crim <- ggplot(aes(x = crim, fill = ..x..), data = Boston) + 
  geom_histogram(binwidth = 5) + 
  stat_bin(binwidth = 5, geom="text", 
           aes(label = ifelse(..count..,..count..,NA)), vjust = -.5) +
  theme_classic() + 
  scale_fill_continuous(name = "crim")  +
...
<center>
```{r, echo = FALSE, fig.width = 7}
gg.tax <- ggplot(aes(x = tax, fill = ..x..), data = Boston) + 
  geom_histogram(binwidth = 30) + 
  stat_bin(binwidth = 30, geom="text", aes(label = ..count..), vjust = -.5) + 
  theme_classic() + 
  scale_fill_continuous(name = "tax") +
  guides(fill = guide_legend(ncol = 2))
